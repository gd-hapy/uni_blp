<template>
	<view class="video-contain" v-if="JSON.stringify(current_video_list_info)!=='{}'">
		...
		<uni-nav-bar v-if="this.current_video_list_info.info[0].video.length > 1" left-icon="arrowleft" color="white"
			background-color="#0a2e38" right-text="菜单" @clickLeft="back" @clickRight="showMenu" />

		<video class="video-player" :src="current_player_url" autoplay controls @error="errHandle"
			:poster="poster_image_url"></video>
		<u-popup v-if="this.current_video_list_info.info[0].video.length > 0" border-radius="10" v-model="show"
			...
			<view class="content" style="display: flex;flex-direction: row;flex-wrap: wrap;background-color: #0a2e38;">
				<block v-for="(item, index) in current_video_list_info.info[0].video" :key="index">
					<view
						style="margin:10px;width: 75px;height: 60rpx;display: flex;border: #2693FF 1px solid;border-radius: 5px;"
						@click="switchCurrentPlaying(item, index)">
						<image v-if="item.includes(current_player_url)" src="../../static/playing.png" mode="aspectFit"
							style=" width: 30rpx;height: 30rpx;margin-top: 20rpx;margin-left: 2px;"></image>
						<text style="margin: auto;font-size: 15px;">第{{item.split('$')[0]}}集</text>
					</view>
				</block>
			</view>
		</u-popup>
...
<script>
	...
	export default {
		...
		data() {
			return {
				current_player_url: '', //当前视频url'http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4', //https://vip.ffzyread1.com/20230705/14839_01547161/index.m3u8',
		...
		methods: {
			...
			requestList() {
				...
				let url = 'api.php?out=jsonp&flag=' + this.current_video_info.flag + '&id=' + this.current_video_info.id
